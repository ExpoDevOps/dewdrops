...
def init_db():
    db_path = '/var/www/dewdrops/dailies.db' if os.path.exists('/var/www/dewdrops') else 'dailies.db'
...
@app.route('/dailies/', methods=['GET', 'POST'])
def index():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('dailies.db' if os.path.exists('dailies.db') else '/var/www/dewdrops/dailies.db')
        c = conn.cursor()
...
def report():
    ...
    for note in notes:
        # Handle old notes (4 cols) vs new (5 cols)
        category = note[1]
